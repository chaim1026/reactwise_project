...
annual = Expenses.objects.filter(user=get_current_user(), category='annual', date__month=datetime.date.today().month, date__year=datetime.date.today().year)
name_list = []
...
#                 reply = reply.upper()
            ...
            #  <div class=" bot_reply">
            #     <!-- {% if reply %} -->
            #     <h6 class="pink-text">BOT:{{reply|linebreaks}}</h6>
            #     <!-- {% endif %} -->
            #     <!-- {% if text %} -->
            #     <div id="text">
            #         <h6 class="teal-text">{{user.first_name|upper}}: {{text|upper}}</h6>
            #     </div>
            #     <!-- {% endif %} -->
            # </div>